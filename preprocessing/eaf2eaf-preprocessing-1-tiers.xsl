...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="tier-names" select="map { 
        ...
        'ft_r' : 'fr-gls-ru',
        'ft_e' : 'fe-gls-en',
        'tx_lat_during_transcription' : 'stl-lit-enh',
        'tx_cyr' : 'st-lit-enh-x-source',
        'ft_r_during_transcription' : 'ltr-lit-ru',
        'ft_r_during_trancription' : 'ltr-lit-ru',
        'lit_trans_e' : 'lte-lit-en',
        'com_m' : 'nto-note-ru'
        }"/>
    
    <xsl:variable name="tiers-2nd-speaker" select="('tx_lat-ZN')"/>
    
    <xsl:variable name="speakers-2nd-speaker" select="map {'tx_lat-ZN': 'BZN'}"/>

    <xsl:variable name="tiers-append" select="map {
        'tx_lat_during_transcription' : ('tx_lat','tx_lat-ZN','Enets','Enets-LP','Enets-IP'),
        'tx_cyr' : 'tx_cyr_during_transcription',
        'ft_r': 'ft_r-ZN',
        'ft_r_during_transcription' : ('lit_trans','Russian','Russian-LP','Russian-ip'),
        'com_m' : ('add_ling','add-ling','ASK','Comments','cmt_e','cmt_r','questions')
        }"/>
    
    <xsl:variable name="tiers-skip" select="('speaker','recorder','translator','annotator_preliminary','annotator_toolbox',
        'ft_r-ZN','tx_lat','tx_lat-ZN','Enets','Enets-LP','Enets-IP', 
        'tx_cyr_during_transcription','lit_trans','Russian','Russian-LP','Russian-ip',
        'add_ling','add-ling','ASK','Comments','cmt_e','cmt_r','questions',
        'info','block','de','ELANMediaURL','ELANMediaMIME','ELANMediaExtracted')"/>
