...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="word[matches(item[@type='txt' and @lang='enh']/text(),'^'||$non-punct-regex||'.*'||$punct-regex||'$')]">
        <!-- take all except trailing punctuation into main word -->
        <xsl:variable name="word" select="replace(item[@type='txt' and @lang='enh']/text(),$punct-regex||'+$','')"/>
        <!-- take all except leading non-punctuatin into punct word -->
        <xsl:variable name="punct" select="replace(item[@type='txt' and @lang='enh']/text(),'^'||$non-punct-regex||'+','')"/>
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <item type="txt" lang="enh"><xsl:value-of select="$word"/></item>
            <xsl:apply-templates select="* except item[@type='txt' and @lang='enh']"/>
        </xsl:copy>
        <word>
            <item type="punct" lang="enh"><xsl:value-of select="$punct"/></item>
        </word>
    </xsl:template>

    <!-- If a word is all in brackets, mark whole word as punct and change writing system -->
    <xsl:template match="word[matches(item[@type='txt' and @lang='enh']/text(),'^\(+'||$non-punct-regex||'.+\)\.?$')]">
        <xsl:variable name="word" select="item[@type='txt' and @lang='enh']/text()"/>
        <word>
            <item type="punct" lang="ru"><xsl:value-of select="$word"/></item>
        </word>
    </xsl:template>
    
    <!-- If a word has nothing but punctuation, mark whole word as punct -->
    <xsl:template match="word[matches(item[@type='txt' and @lang='enh']/text(),'^'||$punct-regex||'+$')]">
        <xsl:variable name="word" select="item[@type='txt' and @lang='enh']/text()"/>
        <word>
            <item type="punct" lang="enh"><xsl:value-of select="$word"/></item>
        </word>
    </xsl:template>
    ...
    <xsl:template match="phrase/@speaker">
        <xsl:variable name="stl-text" select="../item[@lang='enh-x-source' and @type='lit']/text()"/>
    ...
    <xsl:template match="phrase">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <item type="txt" lang="enh">
                <xsl:value-of select="if (words/word) 
                    then string-join((.//word/item[@type='txt' and @lang='enh']),' ') 
                    else '((…)).'"/>
            </item>
            <xsl:apply-templates select="*"/>
        </xsl:copy>
