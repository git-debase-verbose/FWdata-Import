...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:key name="mb" match="TIER[@LINGUISTIC_TYPE_REF='mb-cf-enf']//REF_ANNOTATION[not(@PREVIOUS_ANNOTATION)]" use="@ANNOTATION_REF"/>
    ...
    <xsl:template match="TIER[@LINGUISTIC_TYPE_REF='tx-txt-enf']//ANNOTATION_VALUE/text()">
        ...
        <xsl:value-of select="(if ($is-first-word or $is-proper) then upper-case(substring(.,1,1))||substring(.,2) else .)||$add-punct"/>
    </xsl:template>
